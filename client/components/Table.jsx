...
function Table () {
  ...
  const totalExpenses = USDollar.format( Number(totalBudget.replace(/[^0-9.-]+/g,"")) - Number(remainingBalance.replace(/[^0-9.-]+/g,"")) );
  ...
  allExpenses.map((expense) => {
    ...
    rows.push(
      <tr key={expense.category}>
        <td>{expense.category.replaceAll('_', ' ').charAt(0).toUpperCase() + expense.replaceAll('_', ' ').slice(1)}</td>
        <td>{expense.total}</td>
      </tr>
    )
  })
